...
# a)
electron_content_percentage_above_ISS2 = 1 - norm(loc=height_at_average_value, scale=standard_deviation).cdf(H)
...
# d)
delta_I_T_P = 40.3 / (frequency**2 * 3 * 10**8) * 10**16 * TEC * 10**9
print("Ionosfersko kašnjenje na lokaciji P (izračunato EGNOS-om) n"
      "a frekvenciji od f=1176.45 MHz: {:.4f} ns".format(delta_I_T_P))

# e)
delta_I_T_ISS2 = delta_I_T_P * electron_content_percentage_above_ISS2
print("Ionosfersko kašnjenje signala s Galileo satelita "
      "koji se nalazi u zenitnom smjeru iznad ISS2: {:.4f} ns".format(delta_I_T_ISS2))

# f)
delta_I_S_ISS2 = delta_I_T_ISS2 * 3 * 10**8 * 10**-9
